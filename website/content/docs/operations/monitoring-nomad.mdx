...
package contains a number of useful tools. Some example to look for:
...
may be a job with a large jobspec or dispatch payload that is
increasing the time it takes to apply raft messages.
...
following metrics allow an operator to observe changes in throughput
at the various points in the scheduling process:
...
- **nomad.broker.total_blocked** - The number of blocked
  ...
  blocked evaluations may mean that the cluster is low in resources or
  that a job has been submitted that can never have all its
  allocations placed.
...
- **nomad.plan.submit** - The time to submit a scheduler plan from the
  ...
  includes the time from `nomad.plan.evaluate` (above). If this metric
